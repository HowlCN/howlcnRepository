...
module.exports = {
  ...
  devServer: {
    ...
    proxy: {
      "/api/*": {
        target: "http://www.api.zhuishushenqi.com", // 目标服务器地址
      ...
      "/b/*": {
        target: "http://www.b.zhuishushenqi.com", // 目标服务器地址
        secure: false, // 目标服务器地址是否是安全协议
        ws: true, //是否代理websocket
        changeOrigin: true, // 是否修改来源, 为true时会让目标服务器以为是webpack-dev-server发出的请求!服务端和服务端的请求是没有跨域的
        pathRewrite: { "^/b": "/" } // 将/api开头的请求地址, /api 改为 /, 即 /api/xx 改为 /xx
      },
      // "/static/*": {
      //   target: "http://statics.zhuishushenqi.com", // 目标服务器地址
      //   secure: false, // 目标服务器地址是否是安全协议
      //   ws: true, //是否代理websocket
      //   changeOrigin: true, // 是否修改来源, 为true时会让目标服务器以为是webpack-dev-server发出的请求!服务端和服务端的请求是没有跨域的
      //   pathRewrite: { "^/static": "/" } // 将/api开头的请求地址, /api 改为 /, 即 /api/xx 改为 /xx
      // },
      "/content/*": {
        ...
        ws: true, //是否代理websocket
        changeOrigin: true, // 是否修改来源, 为true时会让目标服务器以为是webpack-dev-server发出的请求!服务端和服务端的请求是没有跨域的
        pathRewrite: { "^/content": "/" } // 将/api开头的请求地址, /api 改为 /, 即 /api/xx 改为 /xx
      },
      /****** 王宁 ****** */
      //调用网易云音乐接口
      "/music/*": {
        target: "http://uniquezhou.xyz:3000", // 目标服务器地址
        secure: false, // 目标服务器地址是否是安全协议
        ws: true, //是否代理websocket
        changeOrigin: true, // 是否修改来源, 为true时会让目标服务器以为是webpack-dev-server发出的请求!服务端和服务端的请求是没有跨域的
        pathRewrite: { "^/music": "/" } // 将/api开头的请求地址, /api 改为 /, 即 /api/xx 改为 /xx
      },
      //调用数据库
      "/mongoDB/*": {
        target: "http://49.234.207.2:8888", // 目标服务器地址
        secure: false, // 目标服务器地址是否是安全协议
        ws: true, //是否代理websocket
        changeOrigin: true, // 是否修改来源, 为true时会让目标服务器以为是webpack-dev-server发出的请求!服务端和服务端的请求是没有跨域的
        pathRewrite: { "^/mongoDB": "/" } // 将/api开头的请求地址, /api 改为 /, 即 /api/xx 改为 /xx
      }
    }
  },
  // 第三方插件配置
