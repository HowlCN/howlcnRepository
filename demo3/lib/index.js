; (function () {
    ...
    mode.onclick = function (e) {
        ...
        if (curmode == 'dark') {
            ...
            window.color = '#fff';
            window.branches = [];
            var t = new Tree(new Vector(clientW / 2, clientH), Math.round(window.random(6,10)), curmode, window.randomRgba());
            t.render();
        } else {
            window.ctx.clearRect(0, 0, canvas.width, canvas.height);
            canvas.style.backgroundColor = '#fff';
    ...
    var Branch = function (preDot, v, r, m, t, g, pl, trgba) {
        ...
        this.length = 0;
        //当前枝条的目标长度
        this.targetLength = window.random(pl * 3 / 4, pl * 4 / 5);
        //当前树枝的代数
        this.generation = g || 8;
        //树枝偏移方向
        this.theta = t || 0;
    ...
    Branch.prototype = {
        ...
        forkPosInit: function () {
            ...
            this.forkPos[0] = {
                ...
                forkd: random > 0.5 ? -0.5 : 0.5
            }
            this.forkPos[1] = {
                isfork: Math.random() > b ? false : true,
                forkd: random > 0.5 ? 0.4 : -0.4
            }
            this.forkPos[2] = {
    ...
    Tree.prototype = {
        init: function () {
            var b = new Branch(this.rootP, new Vector(0, -2.5), this.rootR, this.treeMode, 0, 1, 220, this.targetRgba)
        }
    }
    ...
    function start(){
        var t = new Tree(new Vector(clientW / 2, clientH), 10 , 'light', window.randomRgba());
        t.render();

        setTimeout(function(){
            press.style.transition = 'all 1.5s ease-in-out';
            press.style.top = '80px';
            press.style.left = canvas.width - 60 +'px';
        },1000);
